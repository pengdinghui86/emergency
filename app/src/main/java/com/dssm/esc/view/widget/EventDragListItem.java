...
public class EventDragListItem extends LinearLayout {
    ...
    public EventDragListItem(Context context) {
        super(context);
        mContext = context;
        initView();
    }
    public EventDragListItem(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        initView();
    }
    private void initView() {
        ...
        //merge进来整个listItem，在这里可以自己定义删除按钮的显示的布局，随便按照的自己喜好修改都行
        mHiddenDragView = View.inflate(mContext, R.layout.item_event_hide_drag, this);

        mScroller = new Scroller(mContext);
        //将隐藏的删除布局的宽度赋值给边界的值，根据自己的需要可以任意的修改
        mDragOutWidth = dip2px(mContext, 120);
        //删除按钮排布在第一个View之后
        mHiddenLayout.layout(mContentView.getRight(), mContentView.getTop(), mContentView.getRight() + mDragOutWidth, mContentView.getBottom());
    }
     ...
     */
    public boolean onDragTouchEvent(MotionEvent event) {
    ...
    private boolean hasMove = false;//该条目是否已经监听过手势的滑动，用来作为判断是否进行条目左右滑动的条件之一
        ...
        if (getScrollX() != 0) {
            ...
            new Handler().postDelayed(new Runnable() {
                public void run() {
                    ...
                    hasMove = false;//状态重置后将是否滑动过置为没有滑动过
                }
            }, 10);
        }
    }
