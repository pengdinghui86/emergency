        ...
        if(db.isOpen()){
            ...
            while(cursor.moveToNext()){
                ...
                if(status == InviteMessage.InviteMesageStatus.BEINVITEED.ordinal())
                    ...
                    if (status == InviteMessage.InviteMesageStatus.BEREFUSED.ordinal())
                        msg.setStatus(InviteMessage.InviteMesageStatus.BEREFUSED);
                    else {
                        if (status == InviteMessage.InviteMesageStatus.AGREED.ordinal())
                            msg.setStatus(InviteMessage.InviteMesageStatus.AGREED);
                        else {
                            if (status == InviteMessage.InviteMesageStatus.REFUSED.ordinal())
                                msg.setStatus(InviteMessage.InviteMesageStatus.REFUSED);
                            else {
                                if (status == InviteMessage.InviteMesageStatus.BEAPPLYED.ordinal()) {
